...
export default function Images({ metadata, sections, categories }: ImagesProps) {
    const [searchQuery, setSearchQuery] = useState('');

    return (
        <Layout title="Image Gallery" sections={sections} metadata={metadata}>
            <div className="w-[80vw] mx-auto space-y-8">
                <div className="pt-4 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900 md:absolute md:left-1/2 md:transform md:-translate-x-1/2">Image Gallery</h1>
                ...
                <Gallery categories={categories} type="images" searchQuery={searchQuery} />
            </div>
        </Layout>
    );
}
...
export async function getStaticProps() {
    return {
        props: {
            ...
            categories: getMediaCategories('images')
        }
    };
}
